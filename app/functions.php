...
if (get_magic_quotes_gpc()) {
    function virtualUndoMagicQuotes($array, $topLevel=true) {
        $newArray = array();
        foreach($array as $key => $value) {
            ...
            $newArray[$key] = is_array($value) ? virtualUndoMagicQuotes($value, false) : stripslashes($value);
        }
        return $newArray;
    }
    $_GET = virtualUndoMagicQuotes($_GET);
    $_POST = virtualUndoMagicQuotes($_POST);
    $_COOKIE = virtualUndoMagicQuotes($_COOKIE);
    $_REQUEST = virtualUndoMagicQuotes($_REQUEST);
}
...
function virtualFindClassFile($class)
{
    if (defined('COMPILER_INCLUDE_PATH')) {
        $classFile = $class.'.php';
    }
 ...
 * @throws Exception
 */
function virtualCoreErrorHandler($errno, $errstr, $errfile, $errline){
...
function virtualDebugBacktrace($return=false, $html=true, $showFirst=false)
{
    $d = debug_backtrace();
    $out = '';
    if ($html) $out .= "<pre>";
...
function virtualSendErrorHeader()
{
    return;
    if (!isset($_SERVER['SCRIPT_NAME'])) {
        return;
    }
    $action = Virtual::app()->getRequest()->getBasePath()."bugreport.php";
    echo '<form id="error_report" method="post" style="display:none" action="'.$action.'"><textarea name="error">';
}

function virtualSendErrorFooter()
{
    return;
    if (!isset($_SERVER['SCRIPT_NAME'])) {
        return;
...
function virtualParseCsv($string, $delimiter=",", $enclosure='"', $escape='\\')
{
...
{
    $xmlArray = simplexml_load_file($filePath, 'SimpleXMLElement', LIBXML_NOCDATA);

    return json_decode(json_encode((array) $xmlArray), true);
}