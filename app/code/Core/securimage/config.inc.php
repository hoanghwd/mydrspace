...
$filePath =  BP . DS . 'app' . DS. 'etc'. DS .'local.xml';
$xmlArray = simplexml_load_file($filePath, 'SimpleXMLElement', LIBXML_NOCDATA);
$crArray = json_decode(json_encode((array) $xmlArray), true);
$credentials = $crArray['global']['resources']['connection'];

return array(
    /**** CAPTCHA Appearance Options ****/
    'image_width'      => 255,       // width of captcha image in pixels
    ...
    'database_host'    => $credentials['host'],     // database server host to connect to

    //'database_user'    => 'root',          // database user to connect as
    'database_user'    => $credentials['username'],          // database user to connect as

    //'database_pass'    => '',              // database user password
    'database_pass'    => $credentials['password'],              // database user password
