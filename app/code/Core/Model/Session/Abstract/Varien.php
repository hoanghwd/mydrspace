...
class Core_Model_Session_Abstract_Varien extends Varien_Object
{
     ...
     * @return Core_Model_Session_Abstract_Varien
     */
    public function start($sessionName=null)
    {
        ...
        switch ($moduleName) {
             ...
             */
            case 'db':
                /* @var $sessionResource Core_Model_Resource_Session */
                $sessionResource = Virtual::getResourceSingleton('core/session');
        ...
        if (Virtual::app()->getStore()->isAdmin()) {
            $sessionMaxLifetime = Core_Model_Resource_Session::SEESION_MAX_COOKIE_LIFETIME;
            $adminSessionLifetime = (int)Virtual::getStoreConfig('admin/security/session_cookie_lifetime');
        ...
        $sessionCacheLimiter = Virtual::getConfig()->getNode('global/session_cache_limiter');
        ...
        if (Virtual::app()->getFrontController()->getRequest()->isSecure() && empty($cookieParams['secure'])) {
            ...
            if (!isset($_SESSION[self::SECURE_COOKIE_CHECK_KEY])) {
                $checkId = Virtual::helper('core')->getRandomString(16);
    ...
    {
        return Virtual::getSingleton('core/cookie');
    }
    ...
    {
        return Virtual::getBaseDir('session');
    }
    ...
    public function getValidatorData()
    {
        ...
        // collect ip data
        if (Virtual::helper('core/http')->getRemoteAddr()) {
            $parts[self::VALIDATOR_REMOTE_ADDR_KEY] = Virtual::helper('core/http')->getRemoteAddr();
        }
        ...
        if (isset($this->_data['visitor_data']['customer_id'])) {
            $parts[self::VALIDATOR_PASSWORD_CREATE_TIMESTAMP] =
                Virtual::helper('customer')->getPasswordTimestamp($this->_data['visitor_data']['customer_id']);
        }
